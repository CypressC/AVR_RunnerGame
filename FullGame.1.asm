...
end_game:
      ...
      RCALL data_wrt
      RCALL delay_seconds
	  RCALL read_EEPROM		;reads from EEPROM
	  SUBI R29,15			; subtracts offset from score
	  CP R29,R30				;compares score to current best
	  BRPL new_best			; If the result is still positive write new best to EEPROM
	  RCALL data_wrt		; otherwise wrt the current best
new_best:
	  mov R30,R29
	  RCALL data_wrt
	  RCALL prog_EEPROM
	  LDI   R17, 120           ;wait 120 cycles
...
gameplay:
      ...
      RCALL LCD_init          ;subroutine to initialize LCD
	 ;-----------------------------------------------------
	  LDI R26,HIGH (31250-1);timer high byte
	  LDI R27,LOW (31250-1)  ;timer low byte
	  ...
	  LDI R17,0x32		 ;keeps offset
	  LDI R29,0b00110000		 ;keeps score
reset:	
	  LDI R18, 0
	  ;SUBI R27,0xA
...
l8: SBIC EECR, 1
    ...
    ;LDI   R30, 0b0011001   ;byte to be written to EEPROM
